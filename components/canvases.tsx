...
export function BeautifulPianoCanvas({startColor, endColor, isVisible}: {startColor: string, endColor: string, isVisible: boolean}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
      ...
      const render = () => {
        if (context.globalAlpha != 0) {
          ...
          for (let i = 0; i < points.length; i++) {
            ...
            const note = i % 12;
            context.strokeStyle = [0,2,4,5,7,9,11].includes(note) ? startColor : endColor
